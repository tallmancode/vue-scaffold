...
export default {
    ...
    mounted () {
        if (!this.isImage) {
            this.$emit('avatar-initials', this.textValue, this.userInitial)
        }
    },
    computed: {
        background () {
            if (!this.isImage) {
                return this.backgroundColor || this.randomBackgroundColor(this.textValue.length, this.backgroundColors)
            }
        },
        fontColor () {
            if (!this.isImage) {
        ...
        userInitial () {
            if (!this.isImage) {
                const initials = this.initials || this.parser(this.textValue, getInitials)
                return initials
            }
            return ''
        }
